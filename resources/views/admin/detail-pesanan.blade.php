    ...
    <div class="max-w-[85rem] w-full mx-auto mt-10">
        <div class="grid grid-cols-4 gap-5 px-5 lg:px-10">
            ...
            <div class="col-span-4 lg:col-span-3 border-2 rounded-lg p-5 lg:p-10 shadow-xl">
                ...
                @if($riwayat->status && !$riwayat->selesai)
                    <div class="flex justify-end">
                        <form id="formKonfirmasiSelesai" action="/confirm-pesanan-selesai/" method="POST">
                            @csrf
                            <input type="hidden" name="nomorPesanan" value="{{ $riwayat->nomorPesanan }}">
                            <button type="submit" class="py-1 px-2 bg-green-500 hover:bg-green-600 text-white font-medium rounded-lg">Konfirmasi Selesai</button>
                        </form>
                    </div>
                @endif
                            ...
                            @if($produk->metodePengiriman)
                                @if(!$produk->resi)
                                    <div class="absolute right-5">
                                        <h1 class="font-light"><b>Resi Kurir:</b><br>
                                        @if(!$riwayat->status)
                                                <input form="formKonfirmasiSelesai" name="resi[]" type="text" class="border-2 px-2 rounded-md font-light text-sm disabled:bg-stone-200" disabled>
                                            @else
                                                <input form="formKonfirmasiSelesai" name="resi[]" type="text" class="border-2 px-2 rounded-md font-light text-sm disabled:bg-stone-200" required>
                                        @endif
                                        </h1>
                                    </div>
                                @else
