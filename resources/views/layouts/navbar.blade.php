...
<div id="hs-overlay-example"
    ...
    <div class="p-4">
        <div
            ...
            @if(\Illuminate\Support\Facades\Auth::check())
                <div
                    ...
                    <button type="button"
                        ...
                        <i class="fa-solid fa-user mr-2 text-stone-600"></i>{{ \Illuminate\Support\Str::limit(auth()->user()->email, 8) }}
                    </button>
            ...
            @else
                <a class="flex items-center gap-x-2 font-semibold text-gray-500 hover:text-blue-600 sm:border-s sm:border-gray-500 sm:my-6 sm:ps-6 dark:border-gray-700 dark:text-gray-400 dark:hover:text-blue-500"
                   href="#">
                    <svg class="flex-shrink-0 w-4 h-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
    ...
    <nav class="relative max-w-[85rem] w-full mx-auto px-4 sm:flex sm:items-center sm:justify-between sm:px-6 lg:px-8"
        ...
        <div class="flex items-center justify-between">
            <a class="flex-none text-xl font-semibold dark:text-white" href="/" aria-label="Brand"><img
                    class="w-32" src="/build/assets/bg-login-aZ_FjiAK.png" alt=""></a>
        ...
        <div class="hs-collapse hidden transition-all duration-300 basis-full grow sm:block">
            <div
                ...
                <a class="font-semibold text-gray-500 hover:text-gray-600 sm:py-6 dark:text-gray-400 dark:hover:text-gray-500"
                   href="/" aria-current="page">Home</a>
                <a class="font-semibold text-gray-500 hover:text-gray-600 sm:py-6 dark:text-gray-400 dark:hover:text-gray-500"
                   href="/products">Product</a>
                <a class="font-semibold text-gray-500 hover:text-gray-600 sm:py-6 dark:text-gray-400 dark:hover:text-gray-500"
                ...
                <a class="relative flex items-center gap-x-2 font-semibold text-gray-500 hover:text-gray-600 sm:border-s sm:border-gray-500 sm:my-6 sm:ps-6 pt-0.5"
                   href="/keranjang">
                    <i class="fa-solid fa-cart-shopping"></i>
                    @if(auth()->check())
                        <span
                            class="absolute -right-4 -top-2 text-xs bg-red-500 text-white px-1 rounded-full">{{ auth()->user()->CountCart() }}</span>
                    @else
                        <span
                            class="absolute -right-4 -top-2 text-xs bg-red-500 text-white px-1 rounded-full">0</span>
                    @endif
                ...
                @if(\Illuminate\Support\Facades\Auth::check())
                    <div
                        ...
                        <button type="button"
                            ...
                            <i class="fa-solid fa-user mr-2 text-stone-600"></i>{{ Str::limit(auth()->user()->email, 10) }}
                        </button>
                ...
                @else
                    <a class="flex items-center gap-x-2 font-semibold text-gray-500 hover:text-gray-600"
                       href="/login">
